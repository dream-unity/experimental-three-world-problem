(() => {
  ...
  function reflect(mode) {
    ...
    state.textContent = mode === 'on' ? 'ON' : mode === 'off' ? 'OFF' : mode === 'loading' ? 'LOADING' : 'PLAY';
  }
  ...
  // Browsers may defer audible autoplay until the first interaction. The score
  // begins on that first gesture unless the visitor explicitly switched it off.
  const unlock = (event) => {
  ...
  document.addEventListener('keydown', (event) => {
    ...
    const buttons = inDetail ? subButtons : worldButtons;
    cycleFocus(buttons, event.key === 'ArrowLeft' || event.key === 'ArrowUp' ? -1 : 1);
