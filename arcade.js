(() => {
  ...
  const BECOME_VERSION = '20260826-become-social-agency-11';
  ...
  const paths = [...basePaths, roleLogicPath, ...perceivePaths, becomeCorePath, becomeDiversityPath, becomeSocialPath];
  ...
  Promise.all(paths.map((path) => fetch(path, { cache: 'force-cache' }).then((response) => {
    ...
    // BECOME loads only its core lifecycle, orthogonal world space and final
    // social-agency planner. No cloud dispatcher or inference runtime is fetched.
