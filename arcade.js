(() => {
  ...
  const BECOME_VERSION = '20260826-become-social-agency-12';
  ...
  const paths = [...basePaths, roleLogicPath, ...perceivePaths, becomeCorePath, becomeDiversityPath, becomeSocialPath, becomeIntegrationPath];
  ...
  Promise.all(paths.map((path) => fetch(path, { cache: 'force-cache' }).then((response) => {
    ...
    // BECOME loads only its core lifecycle, orthogonal world space, social
    // maximin planner and contextual integration layer. No cloud path is loaded.
