...
The browser calls BlockRun's public OpenAI-compatible live endpoint directly. The primary generator is **`nvidia/gpt-oss-120b`**; **`nvidia/step-3.7-flash`** and **`nvidia/mistral-nemotron`** are remote failover models if the primary model is temporarily unavailable. These free models require **no API key, no account and no local model installation**.

The client sends compact live-generation context: summaries of recent generated worlds, numerical training performance and a fresh generation nonce. Personal life-transfer descriptions are not sent. The returned scenario is validated, compared against recent worlds with lexical similarity plus eight relational dimensions, and rejected/regenerated if it is too similar. There is **no stored live fallback**: Become advances only after a genuinely live internet-generated scenario has been received and novelty-checked.
...
- `arcade-parts/become-live-02.txt` — zero-key internet generation, remote model failover, novelty rejection, prefetch and error handling.
...
GitHub Actions validates every pull request. After a push to `main`, it waits for GitHub Pages, compares the live HTML, styling, loader and every active game chunk byte-for-byte against the validated source, performs a browser-style CORS preflight against BlockRun, makes a real zero-key GPT-OSS 120B completion request, verifies that the response is browser-readable, and only then records the deployment as verified-live.
