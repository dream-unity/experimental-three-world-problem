...
The homepage is the **Sovereign Nocturne**: three interdependent mineral fields suspended in a near-black mirrored abyss. They do not form a ring, body, letter, tool or three-product diagram. An asymmetric pressure sheet, an independent counterforce and a low grounding field are built from five cleaved stations with unequal faces, separated by hard corridors of ether. Under pressure they translate toward one short bone-and-coral fracture embedded in the central mineral, materialise as a single resolved event, then release again without surrendering their differences. A displaced reflection persists below the realised matter while memory fibres escape during return. Cyan, emerald and violet survive as rare mineral undertones and embedded portal glints.

A silent 40-second material choreography gathers the field, draws the three folds into authority, subtracts the inherited image, inverts and reconstitutes their mineral faces, ignites the sovereign filament, then returns remembered fibres to possibility. The form never auto-spins. Pointer and touch orbit are restrained, while holding the field visibly compresses the separated worlds toward one event. The inherited predictive ghost is neither obeyed nor destroyed: it is displaced beneath the realised matter and made useful as memory. This is the essay’s meeting ground between worlds—spirit converted to physical consequence, the body retained as an anchor, and physical experience returned to the next dream.

Selecting a world changes viewpoint and accent without splitting the single composition, then reveals its three capacity glints; the DOM labels remain accessible equivalents and open the corresponding games. WebGL2 renders procedurally generated, hard-faceted mineral lamellae, an embedded luminous fracture, custom-shaded reflection and continuous return fibres. If WebGL2 is unavailable, a deliberate Canvas2D three-field composition preserves the same navigation. Context loss is recoverable, and reduced-motion mode fixes one resolved pose while suppressing autonomous deformation and fibres.
