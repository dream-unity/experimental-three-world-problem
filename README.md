...
The homepage retains **I Remember Tomorrow** as its published score and exposes a persistent score control. Its public analyser continues to govern subtle interface phase cues, while the sculpture itself uses one coherent dimensionless motion clock so crown, abyss and return can never contradict one another. Browsers that defer audible autoplay start the published score on the visitor's first interaction. An explicit off choice is remembered locally. No private reference media is named, required or delivered by the site.
