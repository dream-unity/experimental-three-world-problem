...
const tmp=mkdtempSync(join(tmpdir(),'du-objective-'));const fullPath=join(tmp,'complete.js');writeFileSync(fullPath,full);
...
assert.match(loader,/BECOME_VERSION = '20260826-become-objective-16'/);
...
assert.match(index,/arcade\.js\?v=20260826-become-objective-16/);
assert.doesNotMatch(loader,/become-live-02\.txt|vercel|blockrun|groq|web-llm|transformers/i);
for(const marker of ['S14_WINS','S14_OBJECTIVES','SOLVED WHEN','SOCIAL WIN','DID THE STATE','phenomenological completion'])assert.ok(objective.includes(marker),`missing objective marker ${marker}`);
...
vm.runInContext(`${core}\n${diversity}\n${social}\n${integration}\n${compression}\n${rapid}\n${objective}\nthis.definition=GAME_BY_KEY['maker:2'];this.drills=BECOME_DRILLS;this.state=becomeState;this.renderCue=bRenderCue;this.runDrill=bRun;this.renderRating=bRenderRating;`,runtime);
runtime.definition.factory().reset({setScore(){},setMetric(){},sfx(){}});
const api=runtime.window.DREAM_UNITY_BECOME_SOCIAL_AGENCY;
assert.equal(api.profiles,40);assert.equal(api.activationObjectiveVersion,'20260826-activation-objectives-1');assert.equal(api.socialWinCount,40);assert.equal(api.socialWinWordMax,11);assert.equal(api.objectiveWordMax,14);assert.equal(api.solvedWordMax,11);
...
function inspect(queue,label){
  ...
  for(const s of queue){
    ...
    assert.equal(s.__activation14.version,'20260826-activation-objectives-1');
    assert.ok(words(s.__activation14.win)>=5&&words(s.__activation14.win)<=11,`${label} bad social win: ${s.__activation14.win}`);
    assert.ok(words(s.__rapid13.scene)<=34);maxScene=Math.max(maxScene,words(s.__rapid13.scene));
    ...
    for(const metric of runtime.drills){
      ...
      assert.doesNotMatch(o.objective,/describe|explain|understand the instruction/i);
      assert.notEqual(o.objective,o.solved);
    }
  }
...
assert.match(api.objectiveFor('conviction',first[0]).objective,/three motives/i);assert.match(api.objectiveFor('fidelity',first[0]).objective,/still disagree/i);assert.match(api.objectiveFor('integrated',first[0]).objective,/feedback/i);

runtime.state.queue=first;runtime.state.scenarioIndex=0;runtime.state.track='scenario';runtime.state.drillIndex=0;
runtime.renderCue();assert.match(screen.innerHTML,/SOCIAL WIN/);assert.match(screen.innerHTML,/OBJECTIVE/);assert.match(screen.innerHTML,/SOLVED WHEN/);assert.match(screen.innerHTML,/Count only what becomes perceptually/);
runtime.runDrill();assert.match(screen.innerHTML,/ACTIVE OBJECTIVE/);assert.match(screen.innerHTML,/SOLVED WHEN/);
runtime.renderRating();assert.match(screen.innerHTML,/DID THE STATE/);assert.match(screen.innerHTML,/phenomenological completion/);
...
console.log(`Objective Become validated: 40 social wins; 17 experiential mechanisms; scene ≤${a.maxScene}/34; 10 scenarios ${elapsed.toFixed(1)}ms; social min ${a.minSocial.toFixed(3)}; combined min ${a.minCombined.toFixed(3)}; zero immediate repeats.`);
